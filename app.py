...
def add():
    user = []    
    selecao  = request.form.get("Seleção")
    continente  = request.form.get("Continente")
    copas = request.form.get("Número de Copas do Mundo")
    if selecao != '' and continente != '' and copas != '':        
        user.append(selecao.strip())
        user.append(continente.strip())
        user.append(copas.strip())
        lista.append(user)        
        print(f'Add: {lista}')                     
    else:
        print('** Usuario não cadastrado, todos os dados devem ser fornecidos **')    
